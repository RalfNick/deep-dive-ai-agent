...
- 新增测试 `parse_price("￥19.90") == 19.9` 通过；
- 原有货币格式测试不回归；
- 修改只发生在允许路径；
- 补丁能够格式化并通过静态检查；
- Agent 在预算内终止，没有伪造测试结果。

这里沿用第 1 章的 `float` 返回类型。如果业务改用 `Decimal` 表达金额，应单独变更函数合同和测试，不能把返回浮点数的实现直接与 `Decimal("19.90")` 比较。
...
## 进阶阅读：截至 2026-08-09 的前沿观察

第一次阅读可以跳过本节，直接进入本章小结与实验练习。前面已经解释了训练、推理预算与 API 的稳定机制；下面用于查阅具体产品，不要求记住模型名和参数档位。内容保留 2026-08-09 的核对日期，本轮文字修订没有重新验证这些产品状态。
