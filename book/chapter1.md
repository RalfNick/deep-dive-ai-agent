...
在进入术语和公式之前，先沿着下面这张图从 1 走到 4。第一次阅读只需记住：文本先变成 Token 和向量，Transformer 反复处理上下文，模型得到下一个 Token 的概率，再把选中的 Token 追加回上下文。后文会逐层拆开每个阶段。

![大模型如何从输入文本逐个生成 Token](./images/fig1-0-how-llms-work-v2.png)

这张总览图解释的是模型内部的生成主线，不表示模型能够直接执行工具。图中的“工具调用”只是生成循环的一种停止条件；真正的权限判断、执行与验证属于模型之外的 Agent Harness。

图中第三步使用三个候选的玩具词表：读取、调用、删除的 Logit 分别为 `2.0、1.0、0.1`。在温度 `T=1` 时，Softmax 给出约 `0.659、0.242、0.099`，与本章 `sampling_demo.py` 相同；它们不是完整真实词表，也不是工具执行概率。图示这次选中“读取”，第四步就把同一项追加回上下文。分词、ID 和位置矩阵仅作示意；第二步用于认识组件，具体归一化与残差顺序请结合后文的模型结构理解。
