...
设想第 3 章的 Agent 正准备写入补丁，界面弹出“是否允许修改文件”。你暂时离开电脑，进程随后退出。第二天点击批准时，它应该接着写入，重新读取仓库，还是从头再做一次？

再把中断点向后移动一点：补丁已经写入，但“写入成功”的消息尚未返回。重新启动后，系统怎样知道这次动作已经做过？如果它直接再写一次，文件最终可能仍然正确，过程却已经重复了副作用。

这些问题不要求模型先学会更高深的推理。我们首先需要知道：谁保存待批准的动作，谁检查当前文件版本，谁执行写入，谁记录回执，谁决定任务完成。

本章会固定模型决策，用审批、恢复和故障实验逐项观察这些责任。第一个对照仍从“过早宣布完成”开始，随后关注的重点是：把第 3 章已经能工作的循环，拆成可以暂停、恢复和独立检查的外围系统。
