...
![图 10-4：提交先持久化，worker 领取后运行；请求断开不抹掉作业记录，终态由系统确认。](images/fig10-4-durable-job.png)
...
这是完整实验中的计算部分。代码看起来并不像一个“智能系统”，而这正是要点：模型负责理解用户想要哪份报表，不该负责心算所有金额、记忆每条处理进度，或猜测工作有没有提交。确定性业务逻辑由程序执行，模型最终读取经过验证的结果。
