...
先不看代码，试着解释三个故障：JSON 合法却被拒绝、工单已创建却收不到响应、工具结果返回却对应错了调用。它们分别发生在参数与授权、执行结果不确定、调用关联三处，处理办法不能混用。

能定位到这一层，就已经抓住本章的主线。实现时遇到 Output Schema、Resource 控制权、协议版本或日志通道等细节，可以查阅[22 个补充问答](../chapter9/faq.md)；生产迁移的检查项保留在前一节。这里不再重复展开。
