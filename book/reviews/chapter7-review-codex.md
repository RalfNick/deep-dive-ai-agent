...
## 七、2026-08-26 可读性复审与修订

复审日期：2026-08-26

本轮从“第一次系统学习 Agent 的读者”视角重新检查，技术边界仍成立，但主线存在一个明显教学缺口：正文声称逐步升级，实际只显式标出了 v0–v2，Write、Store、Recall、Correct、Forget 没有形成同一条可观察路径。现已完成以下修订：

- 把贯穿实验整理为连续的 v0–v7，每一版只增加一种责任，并各自给出可观察运行结果；
- 把详细召回算法、产品责任映射和生产治理标为“进阶阅读”，第一次阅读可以沿主线直达小结；
- 增加候选记忆、作用域、来源权威、当前有效视图和删除标记的中文先行词表，减少术语切换；
- 在 Store、Recall、Correct 和 Forget 补入最小调用与输出，并说明代码片段共享前置 Fixture，完整入口为 `chapter7/tests/test_runtime.py`；
- 先用样本计数解释 Precision/Recall，再给公式；把本章小结放在 Claims/Non-claims 前，避免教学结论被审计术语打断。

修订后正文约 2.6 万有效中文字符，共 32 个二三级标题；v0–v7 有 8 处显式运行结果。章节仍保留 7 张原创图、5 组无 API Key 实验和 14 道分层练习，没有扩大模型或产品能力声称。

验证结果：

- 新增发布测试冻结 v0–v7 顺序、逐版结果、主线顺序、进阶层级与小结位置；
- 第 7 章离线测试由 64 项增至 65 项，全部通过；
- 三份规范报告内容与 SHA-256 保持不变，说明本轮只改变教学呈现与发布合同，没有改写实验结论。

结论：**通过。** 对零基础读者，主线已从“概念正确但需要自己拼装”改成“先看到失败，再逐版修复”；对工程读者，原有边界、实验限制、来源台账和可运行代码仍完整保留。
