...
> 历史评审存档：下文对应 2026-08-28/29 的旧基线，评分不代表当前候选。2026-09-06 发现并修正的片段事实继承和最终回查问题，见[最新复审记录](./chapter8-review-v1.4-rc2.md)。

核对日期：2026-08-28
v1.2 增补复核：2026-08-29
初审基线：`3e3da5f`（`docs: write chapter 8 rag and knowledge base`）
复审基线：`19569c5`（`build: publish chapter 8 in the book site`）
增补基线：`9f7707b`（`docs: strengthen chapter 8 evidence contracts`，由 `book-chapter8-v1.1` 保留）
审阅范围：`book/chapter8.md`、`chapter8/` 实验、8 幅插图、练习与答案、资料台账和规范报告。

## 结论

通过，综合评价 **9.4/10**。本章已经达到“可以独立阅读、可以复现实验、可以核对证据”的出版状态。
...
复审发现的 P1 已全部修复：原报告有 5 个 `expected_status` 与 `answer_status` 不一致，其中 2 个是无答案夹具合同不完整导致的错误回答，另外 3 个是正文承认、报告却未显式分类的假阴性。v1.1 修复前者，并把后者标为 `failure_probe / false_abstain`；所有符合性案例现在都匹配预期，意外状态偏差和错误放行均为 0。检索指标也统一为“唯一文档 + 固定 K 分母”。历史版本不改写，分别由 `book-chapter8-v1.0` 和 `book-chapter8-v1.1` 保存。

v1.2 参考两篇 RAG 工程长文做了有限增强，没有扩张 v0—v7 主线：图 8-2 与 v2 现在区分 Source Chunk 和派生问答/事实卡，把去重、版本归并与治理放到索引前；Embedding 进阶部分加入 Binary Quantization、Hamming 检索和高精度重排，并明确 32 倍只来自 32 bit/维到 1 bit/维的原始表示比例。两篇社交媒体资料只作为问题发现和讲解启发，技术口径回到 Sentence Transformers 与 Milvus 官方文档，未经独立复现的倍数和厂商案例没有进入本章结论。
...
| P2 | 读者/专家 | 原图 8-2 从“清洗切块”直接进入索引，没有解释相似 Chunk、版本副本和派生问答在检索前怎样处理 | `book/chapter8.md`；图 8-2；[S20] | fixed：重绘离线加工路径，增加 Source Chunk、去重/版本归并/治理与可选派生单元；Citation 仍回到当前有效原文 |
| P2 | 专家/来源 | 原文只用一句“量化方法”带过容量优化，读者容易把二值表示的理论 32 倍误读为完整 RAG 收益 | Embedding 进阶段；练习 10；[S18][S19][S21] | fixed：拆开表示、索引、检索和回答四层指标；练习加入 float32、binary、binary + rerank 消融；Non-claims 禁止端到端倍数外推 |
...
6. 新增的“Source Chunk 与派生知识单元”对照表把上游加工讲得更直观；二值量化也先解释 32 倍从哪里来，再解释为什么不能外推，适合第一次接触检索基础设施的读者。
...
- 派生问答与事实卡被限定为检索入口，不被提升为新的权威来源；源文撤回和摘要变化仍能沿来源映射传播。
- Binary Quantization 被限定为向量表示与候选召回优化，不与模型权重量化、索引总内存或答案质量混淆。
...
- `python -m unittest discover -s chapter8/tests -v`：61 项通过；
- `python scripts/check_repository.py --root . --git-history`：退出码 0；
- 发布合同：有效中文 27,043 字，二/三级标题 42 个，插图 8 幅，练习 14 道，答案 14 道，来源 21 条。
...
资料结构合理：算法概念优先使用 RAG、DPR、RRF、ColBERTv2、Lost in the Middle 和 RAGAS 的原始研究或权威教材；产品与框架行为使用 Anthropic、LangChain/LangGraph、OpenAI 和 Ragas 当前官方资料；二值量化使用 Sentence Transformers 与 Milvus 官方资料；本地实现与作者资料单列，不拿旧文章或社交媒体文章替代当前技术事实。

每条资料都有“事实使用”“明确不声称”“最后核对”和“出版前复核”。这比只列参考链接更有价值，因为它让读者和作者都知道来源能支持到哪里。S08—S14、S18—S21 属于快变资料，出版或大版本发布前仍需重新打开核对；S20、S21 仅是编辑与问题发现来源。本次核对日期不应被理解为长期有效保证。

## 最终评分

| 维度 | 分数 | 说明 |
| --- | ---: | --- |
| 通俗性与阅读路径 | 9.2 | 具体问题驱动；上游加工和 32 倍来源均先给直觉，再给边界 |
| 技术准确性与边界 | 9.6 | 治理、检索、证据、评估与表示优化边界完整；Non-claims 充分 |
| 实验与可复现性 | 9.6 | 真实代码、失败探针、状态分类、稳定报告和脱敏 Trace 形成闭环 |
| 来源质量与时效 | 9.3 | 一手来源承载技术事实；社交媒体资料被明确降级为编辑启发 |
| 练习与教学闭环 | 9.2 | 分层练习覆盖理解、实现、诊断、量化消融和生产设计 |

综合评价：**9.4/10，建议发布 v1.2。**
