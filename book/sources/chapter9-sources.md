...
本台账只记录本章实际使用的一手来源和本地工程证据。网页事实集中核对日期为 2026-09-01；其中协议首页与官方 Python SDK 发布页又于 2026-09-04 复核。协议、SDK 与产品接口在后续出版前仍需再次复核。
...
> 2026-09-04 复核注：官方规范入口仍指向 `2026-07-28`。
...
> 2026-09-04 复核注：官方 Releases 页面仍将 `v2.1.1` 标为 Latest；本章继续精确锁定该版本，不追随未验证的浮动依赖。
...
- 用于：支持 5 组、21 个 Case（20 个 Runtime Observation + 1 个 Specification Fixture）及未测量字段为 `null` 的声明。
