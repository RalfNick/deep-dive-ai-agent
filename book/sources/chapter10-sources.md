...
版本：v1.1；核对日期：2026-09-07；发布日期：2026-09-09。正文由本地实验组织，官方资料用于产品机制与标准语义，未复制书籍文字、原图或厂商评测结论。
...
## v1.1 发布补充（2026-09-09）

本次不改变上述产品事实口径，不新增模型或 SDK 已实测声明。新增 [quickstart.py](../../chapter10/quickstart.py) 展示完整本地报表调用顺序，[test_quickstart.py](../../chapter10/tests/test_quickstart.py) 通过真实模块命令核验状态、进度和 6,000 分结果。[test_jobs.py](../../chapter10/tests/test_jobs.py) 新增截止时间后同意图恢复、参数冲突、新作业期限拒绝及原预算不被覆盖的回归检查。

明确 `finish()` 检查提交资格与事务一致性，不验证报表业务内容；当前业务结果由固定测试核验。新增状态表与图内“常见状态路径”说明。原五组规范报告保持原字节：新增缺陷证据来自专项回归测试，不冒称旧报告已经覆盖该反例。修订情况见 [v1.1 复审记录](../reviews/chapter10-review-v1.1.md)。
