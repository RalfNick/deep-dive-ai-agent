...
| v1.1 | 2026-09-09 | `book-chapter10-v1.1` | 当前发布版；修复截止时间后同意图重试无法找回既有作业，新增完整 quickstart，明确提交资格与业务正确性边界，补充状态表、版本化图 10-4、练习答案与复审记录；v1.0 不改写 | 第 10 章 42 项测试、仓库 43 项合同、4 项 Node 渲染合同通过；原规范报告逐字节复现；双宽度预览、发布安全检查与 MkDocs strict 构建通过 | 未生成独立 PDF；网站与 Git tag 为本章发布载体 |

v1.0-rc1 是正式发布前的冻结候选，未单独创建 tag；其两个提交仍可从 Git 历史恢复。v1.0 只增加发布所需元数据、导航和门禁，不重写候选提交。第十章未生成或发布 PDF/EPUB。

### v1.1 发布说明（2026-09-09）

保存分支：`codex/chapter10-review-v1.1`。基线为 `book-chapter10-v1.0`（`c50a5ec`），旧 tag、旧图和旧 Review 保留。

- 修复同意图在原截止时间后重试被错误拒绝；新意图继续受期限与容量约束，同键参数冲突不绕过，重试不改写原预算。
- 新增 `chapter10/quickstart.py`，用完整报表流程连接提交、领取、计算、进度、提交与查询。
- 区分提交资格验证和业务正确性，新增状态转移表；图 10-4 使用版本化新文件并注明常见路径，补齐参考答案。
- 验证：42 项章节测试、43 项仓库合同、4 项 Node 渲染合同通过；原规范报告逐字节复现；1280px/390px 预览六图加载、片段链接与整页宽度检查通过；MkDocs strict 构建通过。
- 正式版本由 `book-chapter10-v1.1` 固定，公开 manifest 升级为 v0.10.1；发布后分别验收 GitHub Pages 与独立阅读站点。
