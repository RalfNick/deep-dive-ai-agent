...
先用本章实验检查四件事：工具结果是否进入下一轮决策，失败后策略是否改变，运行是否会停止，完成证据是否对应当前文件。能沿着 Trace 回答这四问，就理解了最小闭环。

如果要让任务跨进程等待审批、在崩溃后恢复，或把工具执行放进沙箱，还需要分离各项运行责任。[第 4 章](./chapter4.md)会在同一价格修复任务上展开这些机制。这里先不增加完整生产清单；工具规模、评估和诊断会在对应章节继续深入。
