...
取三篇只有三个词项的短文，标题和上下文前缀不参与评分。使用仓库分词器，英文转小写，版本号 `3.2` 保留为一个词项：

| 文档 | 正文 | 分词 | 长度 |
| --- | --- | --- | ---: |
| D1 | `3.2 Team SAML` | `3.2, team, saml` | 3 |
| D2 | `3.2 OIDC OIDC` | `3.2, oidc, oidc` | 3 |
| D3 | `2.8 Enterprise SAML` | `2.8, enterprise, saml` | 3 |

查询为 `3.2 Team SAML`，总文档数 \(N=3\)，平均长度 \(avgdl=3\)，取 \(k_1=1.5,b=0.75\)。这是仅用于计算的词项样本，不表示三篇文档都通过真实 RAG 的版本与权限过滤。
...
| 查询词 | 出现于哪些文档 | df | IDF |
| --- | --- | ---: | ---: |
| `3.2` | D1、D2 | 2 | 0.4700036292 |
| `team` | D1 | 1 | 0.9808292530 |
| `saml` | D1、D3 | 2 | 0.4700036292 |

注意 `df` 数的是“有这个词的文档数”，不是词出现的总次数。三篇长度相等，查询词在命中文档中的词频均为 1，因此词频项为：

\[
\frac{f(k_1+1)}{f+k_1(1-b+b\cdot dl/avgdl)}
=\frac{1\cdot2.5}{1+1.5(1-0.75+0.75\cdot1)}=1
\]

所以每个命中词贡献一个 IDF，未命中的词贡献 0：

| 文档 | `3.2` 贡献 | `team` 贡献 | `saml` 贡献 | 总分 |
| --- | ---: | ---: | ---: | ---: |
| D1 | 0.4700036292 | 0.9808292530 | 0.4700036292 | **1.9208365115** |
| D2 | 0.4700036292 | 0 | 0 | **0.4700036292** |
| D3 | 0 | 0 | 0.4700036292 | **0.4700036292** |

排名为 D1、D2、D3；D2 和 D3 同分，本实现按稳定 Chunk ID 排序。D2 中 OIDC 出现两次，但查询没有 OIDC，因此不产生匹配分。

从仓库根目录运行：

```powershell
python -m chapter8.experiments.worked_scores
```

这个入口调用真实的 `BM25Index.rank()` 与 `reciprocal_rank_fusion()`，只打印结果，不访问网络、不写入规范报告。输出的 `bm25` 三项分数应与上表一致，误差小于 `1e-8`。删除查询中的 Team 后，D1 降为 `0.9400072585`，D2、D3 不变；D1 与 D2 的差距从 `1.4508328823` 降为 `0.4700036292`。这体现了稀有词 Team 的贡献，不代表分数就是答案正确概率。
...
令两条列表分别为 `A, B, C, D` 与 `C, B, A, D`，两条通道权重相同，名次从 1 开始，\(k=60\)：

| Chunk | 两路名次 | 计算式 | RRF |
| --- | --- | --- | ---: |
| A | 1 / 3 | `1/61 + 1/63` | 0.0322664585 |
| B | 2 / 2 | `1/62 + 1/62` | 0.0322580645 |
| C | 3 / 1 | `1/63 + 1/61` | 0.0322664585 |
| D | 4 / 4 | `1/64 + 1/64` | 0.0312500000 |

结果为 **A、C、B、D**。A 与 C 同分，稳定 ID 使 A 排在 C 前；它们都略高于 B，不能只按四舍五入后的四位小数比较。交换两条通道在字典中的插入顺序，结果仍应相同。

上面的 `worked_scores` 命令同时打印 `rrf`、`rrf_reordered_channels` 和 `rrf_one_channel`。删除第二条通道后，四个分数依次为 `0.0163934426、0.0161290323、0.0158730159、0.0156250000`，排序恢复为 A、B、C、D。分数下降是贡献项减少，不是答案可信度减半。

可以直接验证：

```powershell
python -m unittest chapter8.tests.test_worked_scores -v
```
