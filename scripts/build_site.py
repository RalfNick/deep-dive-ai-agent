...
def _output_relative(source_relative: Path) -> Path:
    ...
    if (
        ...
        and re.fullmatch(r"chapter[1-6]", source_relative.parts[0])
...
def _allowlisted_sources(root: Path) -> tuple[Path, ...]:
    ...
    explicit += tuple(f"book/chapter{number}.md" for number in range(1, 7))
    explicit += tuple(f"chapter{number}/README.md" for number in range(1, 7))
    explicit += tuple(
