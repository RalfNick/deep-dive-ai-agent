...
Bottom takeaway: “模型每次只前进一步；连贯文本来自自回归循环。”

Constraints: exact Chinese typography; no misspellings; no garbled characters; no duplicated stages; no extra English prose; no equations; no logos; no watermarks; no social media handles; no brand names. Avoid dense paragraphs and tiny unreadable text. Keep the model mechanism accurate and do not depict the LLM as directly executing tools.
