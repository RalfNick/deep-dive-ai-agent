...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_required_roots_exist(self) -> None:
        ...
        expected.extend(f"chapter{number}" for number in range(1, 9))
        self.assertEqual([], [name for name in expected if not (ROOT / name).is_dir()])

    def test_eight_chapters_have_prose_and_experiment_index(self) -> None:
        missing = []
    ...
    def test_book_index_defines_the_published_reading_order(self) -> None:
        ...
        expected.extend(f"(./chapter{number}.md)" for number in range(1, 9))
